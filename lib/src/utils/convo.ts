...
import { ConvoChatWrapper, ConvoInviteWrapper, ConvoManager } from "skychat-lib";
  ...
  async inviteUserToGroup(receiverId: string, groupId: GroupId, serializedKeyPackage: ArrayBuffer): Promise<void> {
    ...
    const response = await fetch(`${this.serverAddress}/api/invite_user`, {
      ...
      body: JSON.stringify({
        ...
        // welcome_message: this.toUrlSafeB64(groupInvite.welcomeMessage),// TODO: this is broken
